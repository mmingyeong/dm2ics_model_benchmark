...
class ViT3D(nn.Module):
    def __init__(
        ...
        assert ih % ph == 0 and iw % pw == 0 and frames % pf == 0, "Image size must be divisible by patch size"

        # Patch 구조
        patch_dim = in_channels * pf * ph * pw
        dh, dh, dw = frames // pf, ih // ph, iw // pw
        num_patches = dh * dh * dw
        self.patch_dims = (dh, dh, dw)
    ...
    def forward(self, x):
        ...
        assert x.shape[1] == self.pos_embedding.shape[1], \
            f"Mismatch in number of patches: got {x.shape[1]} vs {self.pos_embedding.shape[1]}"
        ...
        # Voxel 복원
        x = self.to_voxel_patch(x)  # [B, N, out_channels * pd*ph*pw]
        ...
        x = x.view(b, self.out_channels, d * pd, h * ph, w * pw)
        return x
