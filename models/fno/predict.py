"""
models/fno/predict.py

Run inference using a trained Fourier Neural Operator (FNO) model and save predictions to HDF5,
...
def run_prediction(input_dir, output_dir, model_path, device="cuda", batch_size=4):
    ...
    model_path : str
        Path to trained FNO model (.pt file).
    device : str
        "cuda" or "cpu".
    batch_size : int
    ...
    logger.info(f"📦 Loading FNO model from: {model_path}")
    model = FNO(
        in_channels=1,
        out_channels=1,
    ...
    test_input_files = all_input_files[10:12]  # 테스트 split 기준

    logger.info(f"🧪 Selected {len(test_input_files)} test input files.")
    logger.debug("📁 Input files:\n" + "\n".join(test_input_files))

    for input_path in tqdm(test_input_files, desc="🚀 Running FNO predictions"):
        ...
        if x.ndim == 4:
            x = x[:, None]  # Add channel dimension → [N, 1, D, H, W]
        elif x.ndim != 5:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run FNO inference on test files, preserving filenames.")
